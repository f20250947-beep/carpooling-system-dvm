...
def is_within_range(route_nodes, target_node):
    for node in route_nodes:
        path = find_route(node, target_node)
...
def driver_dashboard(request):
    ...
    return render(request, 'accounts/driver_dashboard.html', {'trips': trips})
...
def driver_requests(request):
    ...
    for trip in active_trips:
        remaining_nodes = list(
            trip.route.filter(is_passed=False).values_list('node', flat=True)
        )
        ...
        for req in pending_requests:
            ...
            if (
                is_within_range(remaining_nodes, req.pickup_node) and
                is_within_range(remaining_nodes, req.dropoff_node)
            ):
                detour, fare = calculate_detour(
                    trip,
                    req.pickup_node,
                    req.dropoff_node
                )
...
def make_offer(request, request_id):
    ...
    if active_trips.exists():
        ...
        if not DriverOffer.objects.filter(
...
def confirm_offer(request, offer_id):
    ...
    driver_wallet, _ = Wallet.objects.get_or_create(user=offer.driver)
